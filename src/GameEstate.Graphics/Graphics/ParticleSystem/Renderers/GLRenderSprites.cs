...
{
    public class GLRenderSprites : IParticleRenderer
    {
        ...
        public GLRenderSprites(IDictionary<string, object> keyValues, IGLContext context)
        {
            _shader = context.LoadShader("vrf.particle.sprite", new Dictionary<string, bool>());
            _quadIndices = context.QuadIndices;
            ...
            {
                var (texture, info) = context.LoadTexture(keyValues.Get<string>("m_hTexture"));
                _texture = texture;
                _textureSequences = info?.Get<TextureSequences>("sequences");
            }
            else _texture = context.ErrorTexture;
