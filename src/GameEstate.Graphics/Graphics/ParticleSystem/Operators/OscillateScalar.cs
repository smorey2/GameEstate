    ...
    public class OscillateScalar : IParticleOperator
    {
        ...
        public void Update(Span<Particle> particles, float frameTime, ParticleSystemRenderState particleSystemState)
        {
            ...
            for (var i = 0; i < particles.Length; ++i)
            {
                ...
                if (_outputField == ParticleField.Radius)
                    particles[i].Radius += delta * rate * frameTime;
                else if (_outputField == ParticleField.Alpha)
                    particles[i].Alpha += delta * rate * frameTime;
                else if (_outputField == ParticleField.AlphaAlternate)
        ...
        {
            if (_particleRates.TryGetValue(particleId, out var rate))
                return rate;
            else
            {
                var newRate = _rateMin + ((float)_random.NextDouble() * (_rateMax - _rateMin));
                _particleRates[particleId] = newRate;
                return newRate;
            }
        }
        ...
        {
            if (_particleFrequencies.TryGetValue(particleId, out var frequency))
                return frequency;
            else
            {
                var newFrequency = _frequencyMin + ((float)_random.NextDouble() * (_frequencyMax - _frequencyMin));
                _particleFrequencies[particleId] = newFrequency;
                return newFrequency;
            }
        }
    }
