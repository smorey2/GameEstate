    ...
    public class PakBinaryTes : PakBinary
    {
        ...
        public unsafe override Task ReadAsync(BinaryPakFile source, BinaryReader r, ReadStage stage)
        {
            ...
            else if (string.Equals(Path.GetExtension(source.FilePath), ".dat", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (header.DataSize != r.BaseStream.Length)
                    throw new InvalidOperationException("File is not a valid bsa archive.");
            ...
            if (magic == F4_BSAHEADER_FILEID)
            {
                ...
                else if (file.FileInfo is F4_HeaderGNMF gnmf)
                {
                    var s = new MemoryStream();
                    {
                        ...
                        w.Write(UnsafeUtils.ReadBytes(gnmf.Header, 32));
                        for (var i = 0; i < 208; i++)
                            w.Write((byte)0x0); // Padding
