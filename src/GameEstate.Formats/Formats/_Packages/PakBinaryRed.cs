...
using GameEstate.Formats.Aurora;
        ...
        public unsafe override Task ReadAsync(BinaryPakFile source, BinaryReader r, ReadStage stage)
        {
            ...
            switch (magic)
            {
                ...
                case RDAR_MAGIC: // Signature("RDAR")
                    {
                        var hashLookup = Resources.Red.CP77.HashLookup;
                        ...
                        for (var i = 0; i < headerFiles.Length; i++)
                        {
                            ...
                            if (nameHashs.Contains(hash))
                            {
                                Console.WriteLine($"File already added in Archive {source.FilePath}: hash {hash}, idx {i}");
                                continue;
                            }
                            nameHashs.Add(hash);
        ...
        public unsafe override Task<Stream> ReadDataAsync(BinaryPakFile source, BinaryReader r, FileMetadata file, Action<FileMetadata, string> exception = null)
        {
            ...
            switch (source.Version)
            {
                ...
                case 'T':
                    ...
                    return Task.FromResult(fileData);
                default:
                    throw new ArgumentOutOfRangeException(nameof(source.Version), $"{source.Version}");
            }
        }
    }
}