...
{
    public class OctreeDebugRenderer<T>
        where T : class
    {
        ...
        public OctreeDebugRenderer(Octree<T> octree, GuiContext guiContext, bool dynamic)
        {
            _octree = octree;
            _dynamic = dynamic;

            _shader = _shader = guiContext.ShaderLoader.LoadShader("vrf.grid", new Dictionary<string, bool>());
