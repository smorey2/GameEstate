...
namespace GameEstate.Toy.Renderer
{
    ...
    {
        readonly Dictionary<string, RenderMaterial> Materials = new Dictionary<string, RenderMaterial>();
        readonly GuiContext GuiContext;
        int ErrorTextureID;
        public static int MaxTextureMaxAnisotropy { get; set; }

        public MaterialLoader(GuiContext guiContext) => GuiContext = guiContext;
        ...
        public RenderMaterial GetMaterial(string name)
        {
            ...
            var resource = GuiContext.LoadFileByAnyMeansNecessary(name + "_c");
        ...
        public RenderMaterial LoadMaterial(object resource)
        {
            if (resource == null)
            {
                var errorMat = new RenderMaterial(new Material());
            ...
            var mat = new RenderMaterial((Material)resource.DataBlock);
        ...
        {
            var textureResource = GuiContext.LoadFileByAnyMeansNecessary(name + "_c");
