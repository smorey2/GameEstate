...
namespace GameEstate.Toy.ParticleRenderer.Initializers
{
    public class RandomRotation : IParticleInitializer
    {
        ...
        public RandomRotation(Dictionary<string, object> keyValues)
        {
        ...
        public Particle Initialize(ref Particle particle, ParticleSystemRenderState particleSystemRenderState)
        {
            ...
            if (_randomlyFlipDirection && _random.NextDouble() > 0.5)
                degrees *= -1;

            if (_fieldOutput == 4)
                particle.Rotation = new Vector3(particle.Rotation.X, particle.Rotation.Y, degrees * PiOver180); // Roll
            else if (_fieldOutput == 12)
                particle.Rotation = new Vector3(particle.Rotation.X, degrees * PiOver180, particle.Rotation.Z); // Yaw
