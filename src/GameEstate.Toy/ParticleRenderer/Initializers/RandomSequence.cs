...
namespace GameEstate.Toy.ParticleRenderer.Initializers
{
    public class RandomSequence : IParticleInitializer
    {
        ...
        {
            if (_shuffle)
                particle.Sequence = _random.Next(_sequenceMin, _sequenceMax + 1);
            else
                particle.Sequence = _sequenceMin + (_sequenceMax > _sequenceMin ? (_counter++ % (_sequenceMax - _sequenceMin)) : 0);
