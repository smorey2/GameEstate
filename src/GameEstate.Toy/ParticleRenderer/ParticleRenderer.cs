...
using GameEstate.Toy.ParticleRenderer.Operators;
using GameEstate.Toy.ParticleRenderer.Renderers;
using GameEstate.Toy.Renderer;

namespace GameEstate.Toy.ParticleRenderer
{
    internal class ParticleRenderer : IRenderer
    {
        ...
        public Vector3 Position
        {
            ...
            set
            {
                ...
                foreach (var child in _childParticleRenderers)
        ...
        public ParticleRenderer(ParticleSystem particleSystem, GuiContext guiContext, Vector3 pos = default)
        {
        ...
        void InitializeParticle(ref Particle p)
        {
            ...
            foreach (var initializer in Initializers)
                initializer.Initialize(ref p, _systemRenderState);
        }
        ...
        public void Update(float frameTime)
        {
            ...
            foreach (var particleOperator in Operators)
                particleOperator.Update(_particleBag.LiveParticles, frameTime, _systemRenderState);
        ...
        void SetupRenderers(IEnumerable<Dictionary<string, object>> rendererData)
        {
            ...
            foreach (var rendererInfo in rendererData)
            {
                ...
                if (ParticleControllerFactory.TryCreateRender(rendererClass, rendererInfo, _guiContext, out var renderer)) renderers.Add(renderer);
        ...
        void SetupChildParticles(IEnumerable<string> childNames)
        {
            ...
            {
                var childResource = _guiContext.LoadFileByAnyMeansNecessary(childName + "_c");
                var childSystem = (ParticleSystem)childResource.DataBlock;

                _childParticleRenderers.Add(new ParticleRenderer(childSystem, _guiContext, _systemRenderState.GetControlPoint(0)));
            }
        }
    }
}
