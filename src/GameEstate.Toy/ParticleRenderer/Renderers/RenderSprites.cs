...
namespace GameEstate.Toy.ParticleRenderer.Renderers
{
    public class RenderSprites : IParticleRenderer
    {
        ...
        readonly int _glTexture;

        readonly Texture.SpritesheetData _spriteSheetData;
        ...
        public RenderSprites(IDictionary<string, object> keyValues, GuiContext guiContext)
        {
            ...
            {
                var (TextureIndex, TextureData) = LoadTexture(keyValues.Get<string>("m_hTexture"), guiContext);
                _glTexture = TextureIndex;
                _spriteSheetData = TextureData?.GetSpriteSheetData();
            }
            else
                _glTexture = guiContext.MaterialLoader.GetErrorTexture();
        ...
        void EnsureSpaceForVertices(int count)
        {
            ...
            if (_rawVertices == null)
                _rawVertices = new float[numFloats];
        ...
        void UpdateVertices(ParticleBag particleBag, Matrix4x4 modelViewMatrix)
        {
            ...
            for (var i = 0; i < particleBag.Count; ++i)
            {
                ...
                // UVs
                if (_spriteSheetData != null && _spriteSheetData.Sequences.Length > 0 && _spriteSheetData.Sequences[0].Frames.Length > 0)
                {
                    var sequence = _spriteSheetData.Sequences[particles[i].Sequence % _spriteSheetData.Sequences.Length];
