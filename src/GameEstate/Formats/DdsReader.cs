...
namespace GameEstate.Formats
{
    ...
    public struct DDSPixelFormat  //: DDS_PIXELFORMAT
    {
        ...
        public DDSPixelFormat(BinaryReader r)
        {
            var size = r.ReadUInt32();
            if (size != 32)
    ...
    public static class DdsReader
    {
        ...
        static byte[] DecodeDXTToARGB(int DXTVersion, byte[] compressedData, uint width, uint height, DDSPixelFormat pixelFormat, uint mipmapCount)
        {
            ...
            {
                var argb = new byte[Texture2DInfo.CalculateMipMappedTextureDataSize((int)width, (int)height, 4)];
        ...
        /// </summary>
        static void ExtractDDSTextureFormatAndData(DDSHeader header, BinaryReader r, out bool hasMipmaps, out uint DDSMipmapLevelCount, out TextureFormat textureFormat, out int bytesPerPixel, out byte[] textureData)
        {
            ...
            if (Utils.ContainsBitFlags((int)header.ddspf.dwFlags, (int)DDSPixelFormats.RGB))
            {
                ...
                else
                {
                    ...
                    else textureData = new byte[Texture2DInfo.CalculateMipMappedTextureDataSize((int)header.dwWidth, (int)header.dwHeight, bytesPerPixel)];
        ...
        static void PostProcessDDSTexture(int width, int height, int bytesPerPixel, bool hasMipmaps, int DDSMipmapLevelCount, byte[] data, bool flipVertically)
        {
            ...
            while (mipMapLevelWidth > 1 || mipMapLevelHeight > 1)
            {
                ...
                if (mipMapLevelIndex + 1 >= DDSMipmapLevelCount)
                    Texture2DInfo.Downscale4Component32BitPixelsX2(data, mipMapLevelDataOffset, mipMapLevelHeight, mipMapLevelWidth, data, mipMapLevelDataOffset + mipMapDataSize);
