        ...
        public unsafe override Task ReadAsync(BinaryPakFile source, BinaryReader r, ReadStage stage)
        {
            ...
            if (version == 2)
            {
                ...
                if (header.OtherMd5SectionSize != 48)
                    throw new InvalidDataException($"Encountered OtherMD5Section with size of {header.OtherMd5SectionSize} (should be 48)");
                ...
                {
                    verification.PublicKey = r.ReadBytes(r.ReadInt32());
                    verification.Signature = r.ReadBytes(r.ReadInt32());
                }
