...
namespace GameEstate.Graphics.Texture
{
    ...
    {
        public static void RGBA16161616F(SKImageInfo imageInfo, BinaryReader r, Span<byte> data)
        {
            ...
            {
                var hr = HalfPrecConverter.ToSingle(BitConverter.ToUInt16(bytes, i + 0));
                var hg = HalfPrecConverter.ToSingle(BitConverter.ToUInt16(bytes, i + 2));
                var hb = HalfPrecConverter.ToSingle(BitConverter.ToUInt16(bytes, i + 4));
            ...
            for (int i = 0, j = 0; i < bytes.Length; i += 8, j += 4)
            {
                ...
                var hg = HalfPrecConverter.ToSingle(BitConverter.ToUInt16(bytes, i + 2));
                var hb = HalfPrecConverter.ToSingle(BitConverter.ToUInt16(bytes, i + 4));
                var ha = HalfPrecConverter.ToSingle(BitConverter.ToUInt16(bytes, i + 6));
