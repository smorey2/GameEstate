...
{
    public class ObjectManager<Object, Material, Texture>
    {
        ...
        public Object CreateObject(string path, out IDictionary<string, object> data)
        {
            ...
            if (!_cachedPrefabs.TryGetValue(path, out var prefab))
                prefab = _cachedPrefabs[path] = LoadPrefabDontAddToPrefabCache(path);
        ...
        public void PreloadObject(string path)
        {
            ...
            if (!_preloadTasks.ContainsKey(path))
                _preloadTasks[path] = _pakFile.LoadFileObjectAsync<object>(path);
        }
