...
namespace WpfTest.Scenes
{
    ...
    /// </summary>
    public class DpiScalingScene : WpfGame
    {
        ...
        Matrix _worldMatrix;
        bool _disposed;
        WpfGraphicsDeviceService _graphicsDeviceService;
        ...
        protected override void Initialize()
        {
            ...
            float tilt = MathHelper.ToRadians(0);  // 0 degree angle. Use the world matrix to tilt the cube along x and y axes.
            ...
            var bottomLeftFront = new Vector3(-1.0f, -1.0f, 1.0f);
            var topRightFront = new Vector3(1.0f, 1.0f, 1.0f);
            var bottomRightFront = new Vector3(1.0f, -1.0f, 1.0f);
            var topLeftBack = new Vector3(-1.0f, 1.0f, -1.0f);
            var topRightBack = new Vector3(1.0f, 1.0f, -1.0f);
            var bottomLeftBack = new Vector3(-1.0f, -1.0f, -1.0f);
            var bottomRightBack = new Vector3(1.0f, -1.0f, -1.0f);
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (_mouseState.LeftButton == ButtonState.Pressed && previous.LeftButton == ButtonState.Released)
            {
    ...
    public class DpiScalingTextComponent : WpfDrawableGameComponent
    {
        SpriteBatch _spriteBatch;
        SpriteFont _font;
        readonly bool _scaleWithDpi;
        readonly Vector2 _position;

        public DpiScalingTextComponent(WpfGame game, bool scaleWithDpi, Vector2 position) : base(game)
        {
        ...
        {
            _spriteBatch.Begin();
            var service = Game.Services.GetService<IGraphicsDeviceService>() as WpfGraphicsDeviceService;
            var scale = _scaleWithDpi ? (float)service.DpiScalingFactor : 1f;
            _spriteBatch.DrawString(_font, $"Dpi scaling is set to {(int)(service.DpiScalingFactor * 100)}%", _position, Color.White, 0f, Vector2.Zero, scale, SpriteEffects.None, 0);
            _spriteBatch.End();
        }
    }
}
